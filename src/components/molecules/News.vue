...
// Extiende JwtPayload para incluir campos personalizados
...
// Obtiene el usuario del token y lo tipa como UserJwtPayload
const newsUser = ref<UserJwtPayload | null>(getUserFromToken() as UserJwtPayload);

// Definición de la interfaz NewsItem
interface NewsItem {
  _id?: string;
  title: string;
  content: string;
...
// Ciclo de vida: carga noticias al montar
onMounted(async () => {
  await fetchNews();
});
...
const publishNews = async () => {
  ...
  if (!title.value.trim() || !content.value.trim()) return displayNotification('Por favor, completa tanto el título como el contenido de la noticia.', 'error');
  ...
  try {
    ...
    if (errorMessage.includes('Sesión expirada')) {
      newsUser.value = getUserFromToken() as UserJwtPayload;
    }
  } finally {
    isLoading.value = false;
  }
...
const editNews = (item: NewsItem) => {
  if (!item._id) return displayNotification('ID de noticia no válido', 'error');
...
const updateNews = async () => {
  ...
  try {
    const updatedNewsItem: NewsItem = { title: title.value.trim(), content: content.value.trim() };
    ...
    displayNotification(errorMessage, 'error');
    if (errorMessage.includes('Sesión expirada')) newsUser.value = getUserFromToken() as UserJwtPayload;
  }
};

const deleteNews = async (id?: string) => {
  if (!newsUser.value) return displayNotification('Debes iniciar sesión para eliminar noticias', 'error');
  if (newsUser.value.rol !== 'admin') return displayNotification('Solo los administradores pueden eliminar noticias', 'error');
  if (!id) return displayNotification('ID de noticia no válido', 'error');
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) throw new Error('Sesión expirada. Por favor, inicia sesión nuevamente.');
      if (response.status === 403) throw new Error('No tienes permisos para eliminar esta noticia.');
      throw new Error(`Error: ${response.status}`);
    }
    await fetchNews();
...
<template>
  ...
  <div id="news-form" class="bg-gray-900 p-3 mb-4 rounded-lg border border-gray-700">
    ...
    <div class="flex justify-end gap-2">
      <button v-if="isEditing" @click="resetForm" class="bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium py-2 px-4 rounded transition duration-200">Cancelar</button>
