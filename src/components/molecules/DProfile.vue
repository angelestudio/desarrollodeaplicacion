...
interface Post {
  ...
  saved?: boolean;
  createdAt?: string; // Added for potential dynamic timestamp
}
...
const formStyles = computed(() => {
  ...
  const colors = {
    green: { border: dark ? 'border-green-700' : 'border-green-500', btnBg: 'bg-green-600 hover:bg-green-700', btnText: 'text-white' },
    yellow: { border: dark ? 'border-yellow-700' : 'border-yellow-500', btnBg: 'bg-yellow-600 hover:bg-yellow-700', btnText: 'text-white' },
    red: { border: dark ? 'border-red-700' : 'border-red-500', btnBg: 'bg-red-600 hover:bg-red-700', btnText: 'text-white' }
  };
...
const notifBtnClass = computed(() => (type: any) => {
  ...
  if (selected) {
    const colors = {
      green: dark ? 'bg-green-600 border-green-600 text-white' : 'bg-green-500 border-green-500 text-white',
      yellow: dark ? 'bg-yellow-600 border-yellow-600 text-white' : 'bg-yellow-500 border-yellow-500 text-white',
      red: dark ? 'bg-red-600 border-red-600 text-white' : 'bg-red-500 border-red-500 text-white'
    };
...
const postStyles = computed(() => ({
  card: isDark.value ? 'bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 text-white shadow-xl hover:shadow-2xl' : 'bg-gradient-to-br from-white to-gray-50 border border-gray-200 text-gray-900 shadow-md hover:shadow-lg',
  statsBar: isDark.value ? 'bg-gray-800/70 text-gray-100 border-gray-600' : 'bg-gray-100/70 text-gray-700 border-gray-200',
  actionButton: {
    like: isDark.value ? 'bg-gray-800 text-green-400 hover:bg-green-900/50 border-green-700 hover:border-green-500' : 'bg-white text-green-600 hover:bg-green-50 border-green-200 hover:border-green-400',
    comment: isDark.value ? 'bg-gray-800 text-blue-400 hover:bg-blue-900/50 border-blue-700 hover:border-blue-500' : 'bg-white text-blue-600 hover:bg-blue-50 border-blue-200 hover:border-blue-400',
    delete: isDark.value ? 'bg-gray-800 text-red-400 hover:bg-red-900/50 border-red-700 hover:border-red-500' : 'bg-white text-red-600 hover:bg-red-50 border-red-200 hover:border-red-400'
  }
}));
...
const ui = computed(() => {
  ...
  return {
    container: dark ? 'bg-black text-white' : 'bg-white text-black',
    contentBg: dark ? 'bg-black' : 'bg-white',
    headerBg: dark ? 'bg-gray-800' : 'bg-gray-300',
    profileBorder: dark ? 'border-white' : 'border-black',
    textPrimary: dark ? 'text-white' : 'text-black',
    textMuted: muted,
    textMutedLight: dark ? 'text-gray-400' : 'text-gray-500',
    statBubble: dark ? 'border-gray-500 text-gray-200' : 'border-gray-400',
    editButton: dark ? 'border-gray-500 text-gray-200 hover:bg-gray-800' : 'border-gray-400 text-gray-700',
    border,
    formInput: input,
    formContainer: dark ? 'border-gray-600 bg-gray-900' : `${border} bg-gray-50`,
    editFormContainer: dark ? 'border-yellow-600 bg-gray-900' : 'border-yellow-400 bg-yellow-50',
    status: {
      success: dark ? 'bg-green-900/70 text-green-200 border border-green-700' : 'bg-green-100 text-green-800',
      error: dark ? 'bg-red-900/70 text-red-200 border border-red-700' : 'bg-red-100 text-red-800'
    },
    loginWarning: dark ? 'border-red-600 bg-red-900/50 text-red-200' : 'border-red-400 bg-red-100 text-red-800',
...
const notifStyles = (type: Notification['type']) => {
  const dark = isDark.value;
  const styles = {
    info: { bg: dark ? 'bg-green-900/30 border border-green-700' : 'bg-green-50 border border-green-200', icon: dark ? 'bg-green-600' : 'bg-green-500' },
    warning: { bg: dark ? 'bg-yellow-900/30 border border-yellow-700' : 'bg-yellow-50 border border-yellow-200', icon: dark ? 'bg-yellow-600' : 'bg-yellow-500' },
    alert: { bg: dark ? 'bg-red-900/30 border border-red-700' : 'bg-red-50 border border-red-200', icon: dark ? 'bg-red-600' : 'bg-red-500' },
    default: { bg: dark ? 'bg-gray-800/70 border border-gray-600' : 'bg-gray-100 border border-gray-300', icon: dark ? 'bg-gray-600' : 'bg-gray-500' }
  };
...
const toastStyles = (type: Toast['type']) => {
  ...
  const dark = isDark.value;
  const styles = {
    info: { bg: dark ? `${base} bg-blue-900/90 border-blue-700 text-blue-100` : `${base} bg-blue-50/95 border-blue-200 text-blue-900`, icon: 'text-blue-500', iconBg: dark ? 'bg-blue-800' : 'bg-blue-100' },
    success: { bg: dark ? `${base} bg-green-900/90 border-green-700 text-green-100` : `${base} bg-green-50/95 border-green-200 text-green-900`, icon: 'text-green-500', iconBg: dark ? 'bg-green-800' : 'bg-green-100' },
    warning: { bg: dark ? `${base} bg-yellow-900/90 border-yellow-700 text-yellow-100` : `${base} bg-yellow-50/95 border-yellow-200 text-yellow-900`, icon: 'text-yellow-500', iconBg: dark ? 'bg-yellow-800' : 'bg-yellow-100' },
    alert: { bg: dark ? `${base} bg-red-900/90 border-red-700 text-red-100` : `${base} bg-red-50/95 border-red-200 text-red-900`, icon: 'text-red-500', iconBg: dark ? 'bg-red-800' : 'bg-red-100' },
    error: { bg: dark ? `${base} bg-red-900/90 border-red-700 text-red-100` : `${base} bg-red-50/95 border-red-200 text-red-900`, icon: 'text-red-500', iconBg: dark ? 'bg-red-800' : 'bg-red-100' }
  };
...
function removeToast(id: string) {
  ...
  if (toast) {
    ...
    setTimeout(() => toasts.value.splice(toasts.value.findIndex(t => t.id === id), 1), 300);
  }
}
...
async function fetchUserData() {
  ...
  try {
    ...
    showStatus(e.message || 'Error al cargar datos del perfil.', false);
    posts.value = [];
    about.value = '';
  }
}
...
function getPersistentNotifs(): Notification[] {
  try {
...
async function fetchNotifs() {
  ...
  try {
    ...
    allNotifs.filter(n => (n.createdAt ? new Date(n.createdAt) : new Date()) > lastCheck && !n.read).forEach(n =>
      showToast(n.title, n.content, n.type as Toast['type'], 8000)
    );
    notifs.value = allNotifs;
    localStorage.setItem(LS_CHECK, new Date().toISOString());
  } catch (e: any) {
    showStatus(e.message, false);
    notifs.value = getPersistentNotifs();
  } finally {
    loading.value = false;
  }
}
...
async function createNotif() {
  if (!isAdmin.value) {
    ...
    showToast('Acceso denegado', 'Solo los administradores pueden crear notificaciones', 'error');
    return;
  }
  loading.value = true;
  try {
...
function startEdit(notif: Notification) {
  editNotif.value = { ...notif };
}
...
async function addComment(postId: string) {
  ...
  try {
    await apiFetch(`/posts/${postId}/comments`, {
      method: 'POST',
      body: JSON.stringify({ content: newComment.value[postId] })
    });
    const post = posts.value.find(p => p._id === postId);
...
function startPolling() {
  ...
  interval = setInterval(() => user.value?.sub && fetchNotifs(), 30000);
}
        ...
        <div v-for="toast in toasts" :key="toast.id" class="pointer-events-auto max-w-sm w-full rounded-xl p-4 transition-all duration-300" :class="[toastStyles(toast.type).bg, toast.isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0', toast.isLeaving ? 'translate-x-full opacity-0' : '']">
          <div class="flex items-start">
            <div :class="[toastStyles(toast.type).iconBg, 'flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center mr-3']">
              <svg class="w-5 h-5" :class="toastStyles(toast.type).icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
