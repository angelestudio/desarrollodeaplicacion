...
const newsItems: Ref<NewsItem[]> = ref([])
const notificationMessage: Ref<string> = ref('')
const notificationType: Ref<'success' | 'error'> = ref('success')
...
const publishNews = async () => {
  ...
  try {
    ...
    // Get the newly created item from server
    const createdItem = await response.json()
    
    // Add to local list at the beginning
    newsItems.value.unshift(createdItem)
...
const deleteNews = async (index: number) => {
  if (index < 0 || index >= newsItems.value.length) return
...
<template>
  ...
  <div id="news-items-container">
    ...
    <div v-else v-for="item in newsItems" :key="item.id" class="bg-gray-200 p-3 mb-4 rounded">
      <div class="flex">
        <div class="mr-3">
