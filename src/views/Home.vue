  ...
  <div class="min-h-screen overflow-y-auto bg-black text-white flex relative">
      ...
      <div class="bg-gray-900 p-6 rounded-xl shadow-lg w-full max-w-md relative overflow-y-auto max-h-[80vh] border border-green-700">
        ...
        <form @submit.prevent="submitPost">
          <div class="mb-3" v-if="!selectedClub">
            ...
            <select v-model="selectedFormClub" class="w-full p-2 rounded bg-gray-800 text-white border border-gray-700 placeholder-gray-500" required>
          ...
          <button type="submit" class="bg-green-600 hover:bg-green-700 px-4 py-2 rounded text-white my-5 transition duration-300">Crear</button>
        </form>
      </div>
    </div>
    ...
    <main class="flex-1 bg-black flex flex-col relative p-4">
      <div class="flex justify-center items-center mb-4 relative">
        <h2 class="text-2xl font-bold text-center w-full text-green-500">Posts</h2>
        <button
          @click="onAddPost"
          title="Agregar post"
          class="absolute right-0 bg-green-600 hover:bg-green-700 rounded-full w-8 h-8 flex items-center justify-center text-white transition duration-300"
        >
          +
        </button>
      </div>
      ...
      <div class="overflow-x-auto whitespace-nowrap mb-4">
        <button
          ...
          class="inline-block px-4 py-2 my-5 mr-2 bg-gray-700 hover:bg-gray-600 rounded-full text-white transition duration-300"
        >
          {{ club }}
        </button>
        <button
          @click="filterByClub('')"
          class="inline-block px-4 py-2 my-5 bg-gray-600 hover:bg-gray-500 rounded-full text-white transition duration-300"
        >
          Todos
        </button>
      </div>
      ...
      <div v-if="filteredPosts.length">
        <div
          ...
          :key="post._id"
          class="bg-gray-900 p-4 rounded mb-2 relative border border-gray-700"
        >
          <h3 class="text-xl font-semibold text-green-400">{{ post.title }}</h3>
          <p class="text-gray-300 mb-2">{{ post.content }}</p>
          <span class="text-sm text-gray-500">Club: {{ post.club }}</span>
          <button
            v-if="userRole === 'admin'"
            @click="onDeletePost(post._id)"
            class="absolute top-2 right-2 bg-red-600 hover:bg-red-700 rounded px-2 py-1 text-sm text-white transition duration-300"
          >
            Delete
          </button>
        </div>
